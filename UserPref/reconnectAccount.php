...
// UPDATE 쿼리 준비
$sql = "UPDATE userAuth SET connect = :connect, disconnect_date = null
...
if ($result) {
    ...
    if ($stmt->rowCount() > 0) {
    echo json_encode(["success" => true, "message" => "재연결 요청에 성공하였습니다."]);
    }
    else{
    ...
    echo json_encode(["success" => false, "message" => "재연결 요청에 실패하였습니다."]);
}
